...
def CONCAT_QUOTE_ORDER(df_quote, df_order) :
    ...
    return ORDER_ACCPET_RATE_W, ORDER_ACCPET_RATE_I  ,SC_CODE 
...
def SINGLE_ITEM_RECORD(df_quote, df_order) :
    ...
    return df_quote[["QUOTE_DATE", "QUANTITY", "WIRE_PRICE", "PROFIT_RATE", "EXCHANGE_RATE", "ORDER_QTY", "ORDER_RATE", "CREA_DATE", "SC_CODE"]].sort_values(by="QUOTE_DATE", ascending=True)


# A = SEARCH_THROUGH_ITEM("WSTD212R")
# B = FETCH_DATA()
# print(SINGLE_ITEM_RECORD(A, B))
#C, D, E = CONCAT_QUOTE_ORDER(A, B)
#print(f"接單率為: {C:.2%} & {D:.2%}, 訂單SC : {E}")
