...
class Neural_Network_Arch:
    def __init__(self, config_path='config.json', load_erp_data = True):
        ...
        self.SAVE_FILE = rf'C:\Users\wesley\Desktop\workboard\Volumn_Project\Training History & Model\{self.today}'
        self.model = None
        self.scaler = None
        self.encoder = None
    ...
    def _prepare_feature_NN(self, save_files = True):
        ...
        if save_files : 
            ...
            with open(os.path.join(self.SAVE_FILE , f'encoder_{self.today}.pkl'), 'wb') as f:
    ...
    def _predict_container_volume(self, train=True):
        ...
        if train:
            ...
            self.model.save(os.path.join(self.SAVE_FILE , f"Ver_{self.today}_OnehotE.keras"))
            ...
            history_df.to_excel(os.path.join(self.SAVE_FILE , f'training_history_{self.today}.xlsx'), index=False)
            print(f"Training history saved to training_history_{self.today}_OnehotE.xlsx")
