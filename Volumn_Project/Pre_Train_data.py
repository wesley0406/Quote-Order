...
class FETCH_ERP_BOX():
	...
	def PREPARE_TRAIN_DATA_BOX(self):
		...
		ALL_HISTORY = ALL_HISTORY[ALL_HISTORY["end_code"] != "D"]
...
class FETCH_ERP_SCREW(FETCH_ERP_BOX) :
	...
	def _fetch_screw_volume(self):
		...
		self.ALL_DATA["Head_Type"] = self.ALL_DATA["pdc_1"].apply(lambda x : x[6:9])
		...
		# drop the invalid and washer product
		self.ALL_DATA = self.ALL_DATA.dropna(subset=["Packing_Ratio"], how="all")
		self.Result = self.ALL_DATA[~np.isinf(self.ALL_DATA["Packing_Ratio"])].copy()
		self.Result.to_excel("ALL_DATA.xlsx")
