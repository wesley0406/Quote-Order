...
app.layout = html.Div([
    ...
    html.Div(id='_'),  # Dummy div for refresh callback
    dcc.Store(id='refresh-trigger'),  # For JavaScript refresh
...
def handle_all_inputs(search_clicks, product_clicks, quote_submit, product_submit, 
    ...
    if search_type == 'quotation':
                ...
                'data': [
                    {
                        ...
                        'hovertemplate': (
                            ...
                            "<b>Price:</b> %{customdata[3]:.2f}<br>"
                            "<extra></extra>"),
                        'customdata': result[['WEIGHT', 'WIRE_PRICE', 'EXCHANGE_RATE', 'TOTAL_PRICE_M']].to_numpy()
                    },
                    {
                        'x': result['PRODUCT_CODE'],
                        'y': result['ORDER_QTY'],
        ...
        try:
            ...
            result = TT.SINGLE_ITEM_RECORD_V2(ITEM, ORDER)
            ...
            bar_figure = {
                'data': [
                    ...
                    },
                    {
                        ...
                        'customdata': result[["ORDER_PRICE", "SC_CODE"]].to_numpy()
                    }
                ],
                'layout': {
...
# First callback to clear the stored data
@app.callback(
    Output('_', 'children'),
    Input('clear-graphs-button', 'n_clicks')
)
...
# Second callback to refresh the page using clientside JavaScript
